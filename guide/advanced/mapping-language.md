...
- **Predicate node**. The predicate node only accepts IRIs, both constants (e.g. `<http://www.example.org/library#title>`) and templates (e.g. `<{iri}>`). Note that `a` stands for `rdf:type` (more precisely, for `<http://www.w3.org/1999/02/22-rdf-syntax-ns#type>`).
