    ...
    "relations": [
        {
            ...
            "columns": {
                "added": [
                    {
                        "name": "yearly_profit",
                        "expression": "yearly_income - yearly_spendings"
                    }
                ] 
...
The column `worker_id` is a primary key. The column `museum_id` is a foreign key that references the table `museum`.
...
This should result in a list of *5* manager names.
...
Generally, Ontop can infer many constraints from the base relation used by a lens. For instance, the field `museum_id` in the table `museum` is a primary key, so it is *unique* and *not null*. Our previously defined lens will be able to infer that the output relation is still unique and not null. On the other hand, for composite primary keys, if one part of the composite key is hidden by a lens, then Ontop knows that the remaining part is no longer guaranteed to be unique.
...
For this tutorial, we want to use our expert knowledge of the table `workers` to enforce the following constraints:
...
4. The column `museum_id` is a foreign key that references the table `museum` (We already know that, because it is already defined as a foreign key in the database, but let's still add it for the sake of the exercise).
